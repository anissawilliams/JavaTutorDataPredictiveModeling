...
NAVY_BG     = '#0F172A'  # dark navy background
FOREST_DARK = '#2B5233'  # dark forest green
FOREST_MED  = '#3A6B4A'  # medium forest green
GOLD        = '#BC9000'  # gold accent
SAGE_LIGHT  = '#8FAF97'  # light sage
WHITE       = '#FFFFFF'  # white
GRAY4       = '#8FAF97'  # reuse sage as "gray text"
GRAY6       = '#475569'  # you can keep this or tweak if needed
TEAL        = '#14B8A6'  # optional, if still used anywhere
CORAL       = '#F87171'  # optional, if still used
ACCENT      = '#BC9000'
NAVY        = '#0F172A'
DEEP        = '#2B5233'
...
FEATURE_SET_LABEL = "16-Feature" if len(features) == 16 else f"{len(features)}-Reduced post-leakage set)"
FEATURE_SET_SHORT = "16feat_behavioral" if len(features) == 16 else f"{len(features)}feat_reduced"
...
# fill with median for nan values
...
for name, model in conf_models.items():
    ...
    sns.heatmap(cm, annot=False, cmap=c_map,
...
ax.set_title(f'Actual vs Predicted — {best_reg_name}\n({FEATURE_SET_LABEL})', fontsize=14, fontweight='bold', color=SAGE_LIGHT)
ax.set_xlim(-5, 105)
ax.set_ylim(-5, 105)
ax.legend(facecolor=FOREST_DARK, edgecolor=GRAY6, labelcolor=WHITE, fontsize=11)
...
ax.text(5, 92, f'CV R² = {cv_r2:.3f}\nCV MAE = {cv_mae:.1f}pp\n{FEATURE_SET_SHORT}', fontsize=11,
        color=SAGE_LIGHT, fontfamily='monospace',
        bbox=dict(boxstyle='round,pad=0.5', facecolor=DEEP, edgecolor=GRAY6, alpha=0.9))

plt.tight_layout()
plt.savefig(f'{OUT}/regression_scatter_{FEATURE_SET_SHORT}.png', dpi=150, bbox_inches='tight', facecolor=FOREST_DARK)
plt.close()
...
ax1.hist(residuals, bins=20, color=TEAL, alpha=0.75, edgecolor=TEAL, linewidth=0.5)
ax1.axvline(x=0, color=SAGE_LIGHT, linewidth=2, linestyle='--')
ax1.set_xlabel('Residual (Actual − Predicted)', fontsize=12)
ax1.set_ylabel('Count', fontsize=12)
ax1.set_title(f'Residual Distribution\n({FEATURE_SET_LABEL})', fontsize=13, fontweight='bold', color=SAGE_LIGHT)
ax1.text(0.95, 0.95, f'Mean: {residuals.mean():.1f}\nStd: {residuals.std():.1f}',
         transform=ax1.transAxes, ha='right', va='top', fontsize=11, color=GOLD,
         fontfamily='monospace', bbox=dict(boxstyle='round', facecolor=FOREST_DARK, edgecolor=GRAY6))

ax2.scatter(pred_all, residuals, c=TEAL, alpha=0.5, s=40, edgecolors='white', linewidth=0.3)
ax2.axhline(y=0, color=SAGE_LIGHT, linewidth=1.5, linestyle='--')
ax2.set_xlabel('Predicted Quiz %', fontsize=12)
ax2.set_ylabel('Residual', fontsize=12)
ax2.set_title(f'Residuals vs Predicted\n({FEATURE_SET_LABEL})', fontsize=13, fontweight='bold', color=SAGE_LIGHT)

plt.tight_layout()
plt.savefig(f'{OUT}/regression_residuals_{FEATURE_SET_SHORT}.png', dpi=150, bbox_inches='tight', facecolor=FOREST_DARK)
plt.close()
