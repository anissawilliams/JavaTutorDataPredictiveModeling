...
Features: condition, session_type, duration_seconds, total_messages, avg_response_time
...
# --- CONFUSION MATRICES (individual, one per model) ---
...
for name, model in conf_models.items():
    ...
    # Draw heatmap with NO lines and NO annotation
    ...
    plt.tight_layout()
    plt.savefig(f'{OUT}/confusion_{safe_name}.png',
                dpi=150, bbox_inches='tight', facecolor=NAVY)
    plt.close()
    print(f"  ✓ confusion_{safe_name}.png")
# ═══════════════════════════════════════════════════════════════
# 3. VISUALIZATIONS
# ═══════════════════════════════════════════════════════════════
print(f"\n{'=' * 70}")
...
# --- C. Model comparison — DUAL (existing, unchanged) ---
...
ax.set_title('Regression Models (CV R²)', fontsize=15, fontweight='bold', color=ACCENT)
ax.axhline(y=0, color=GRAY6, linewidth=1)

plt.tight_layout()
plt.savefig(f'{OUT}/regression_comparison.png', dpi=150, bbox_inches='tight', facecolor=NAVY)
plt.close()
...
ax.set_ylim(0, 1.1)
ax.set_title('Classification Models — CV Accuracy vs AUC', fontsize=15, fontweight='bold', color=ACCENT)
ax.legend(facecolor=DEEP, edgecolor=GRAY6, labelcolor=WHITE, fontsize=10)

plt.tight_layout()
plt.savefig(f'{OUT}/classification_comparison.png', dpi=150, bbox_inches='tight', facecolor=NAVY)
plt.close()
...
# --- Save SHAP dependence plot (example: avg_response_time) ---
plt.figure()
shap.dependence_plot("avg_response_time", shap_values, X_train_raw,
                     feature_names=features, show=False)
plt.tight_layout()
plt.savefig(f"{OUT}/shap_dependence_avg_response_time.png", dpi=200, bbox_inches='tight')
plt.close()
print("  ✓ Saved shap_dependence_avg_response_time.png")
